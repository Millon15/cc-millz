...
# so a new plugin's tests are picked up by adding the files and nothing else.
...
# Harness self-tests live at tests/*.bats; plugin suites at plugins/*/tests/*.bats.
test-bats:
	@command -v bats >/dev/null 2>&1 || { echo 'bats-core not installed — run: brew install bats-core'; exit 127; }
	@suites=$$(ls tests/*.bats plugins/*/tests/*.bats 2>/dev/null); \
...
test-ts:
	...
	@suites=$$(ls plugins/*/tests/*.test.ts 2>/dev/null); \
