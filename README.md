...
| [🗣️ peer-chat](plugins/peer-chat/README.md) | Claude Code and Codex talk in one agterm split, each typing into the other's composer; Claude opens the Codex pane itself when a call is better for an adversary. Messages of any length, paste delivery independent of composer occupancy, an enforced ask ledger, proofs over agreed readings. umputun's two-agent-chat recipe, vendored, plus the spawn step |
