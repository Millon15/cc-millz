...
| [🗣️ peer-chat](plugins/peer-chat/README.md) | Two coding agents talk as equals in one agterm split, each typing into the other's composer: Claude + Codex, two Claudes or two Codexes, any model on either side. Either agent opens the other pane itself (codex on gpt-6-astra by default) when a call is better for an adversary. One skill for every harness, no fixed roles, messages of any length, an enforced ask ledger, proofs over agreed readings. umputun's two-agent-chat engine, vendored behind a pane-identity adapter, plus the spawn step |
...
| [🎬 video-reader](plugins/video-reader/README.md) | Fetch, transcribe and read one video of any length from any yt-dlp site or a local file: original-language captions or local whisper.cpp, seek-sampled frames in indexed contact sheets, chapters, dense windows for follow-ups; no limits by default, no cookies, nothing leaves the machine. Extracted from a private monorepo. |
