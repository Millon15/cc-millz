...
- **No narrating comments.** Code self-explains. Docblocks are hints, not docs: method = ≤1-sentence algo summary, and only when the name doesn't already say it (>1 sentence = refactor smell); class = terse pointers (ticket link, a heavily-used collaborator) + visual aids (decision tables, lists). Only in-body comment allowed: a constraint the code can't express (load-bearing ordering, spec/ticket reason). For the sentence you DO keep, load `concise-writing` — it carries the fact test and the stop condition. Ceiling: prose comment lines stay under 10% of the code lines you added. Annotations are code, not prose, and never count against it — but they split two ways: **delete** any `@param`/`@return`/`@var` that merely restates a declared type, and **always declare** `@throws` (incl. one raised by a private helper), `@deprecated`, `@template`, and PHPStan/Psalm shapes (`array<int, X>`, `list<string>`, `array{…}`) — a static analyser reads those, and a missing one is a defect. Full rule in `concise-writing`.
- **Shape a kept comment for the eye.** One clause per line, broken at the comma or semicolon rather than filled to the column limit. Markdown throughout — `*emphasis*` on the pivot word, `**strong**` on the load-bearing claim, backticked identifiers, `-` bullets for a reasoning chain or a set of cases. Use it **even though nothing renders it**: PHPDoc/Javadoc/JSDoc pass `*text*` through as literal asterisks, and that is the point — the reader is a human scanning source, not a rendered popup.
- **Injected services, never static.** In a project with a service container (Symfony DI, Laravel, Nest, Spring), an instance service resolved by the container is THE default for every new class. A `static` method or a static-only class needs a very good reason — and "it has no dependencies yet" is not one, because the caller can no longer substitute it, the class can no longer grow a dependency without a rewrite of every call site, and it breaks D in SOLID. The legitimate reasons are narrow: a pure function of its arguments with no identity (`Money::fromMinor()`), a named constructor / factory method, an enum method, or a call site that genuinely predates the container (legacy procedural code, a trigger file). Encountering an existing static class in code you touch → convert it, then inject.
...
- **SOLID** —
    ...
    * **D**: depend on injected abstractions (constructor DI), not concretions. A static call is the purest violation there is — it hard-wires the concretion into the call site.
