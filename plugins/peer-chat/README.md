...
| script | `scripts/peer-chat-paste.py` | 📄 the multi-line send: loads the transport for its checks, refuses a non-empty composer, enforces the ask ledger (`--slug`), wraps prose, bracketed paste, confirms the tail, submits, confirms the clear |
