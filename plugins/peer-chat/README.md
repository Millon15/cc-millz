...
- **A ledger the user can follow in a thin pane.** Every message is a short emoji ledger, one segment per line: `🎯` claim, `🔎` evidence as `path:line` or a proof path, `📎` an artifact with how to run it and what it showed, `❓` the one question. Lines under 50 characters, one claim per message, no "round n of m" staging. `peer-chat-paste.py` carries the line breaks (a bracketed paste through the clipboard, saved and restored, behind the transport's own composer checks); `peer-chat.py` stays the one-line path. The point is that the human reading both panes can catch a domain mistake and interrupt either side.
...
| script | `scripts/peer-chat-paste.py` | 📄 the multi-line send: loads the transport for its checks, refuses a non-empty composer, bracketed paste, confirms the tail, submits, confirms the clear |
