...
5. **Stage ② review** — if stage ① moved the plan to `docs/plans/completed/`, use that path. Background: `RALPHEX_REPOS="<repos>" RALPHEX_BASELINE_SHA=<sha> RALPHEX_ROOT_HEAD=<sha> [RALPHEX_NO_CODEX=1] ralphex --external-only --skip-finalize --max-iterations <N> <plan>`; progress file `progress-<plan-stem>-codex.txt` (upstream names the external-only log `-codex` whatever the tool; it is overwritten on relaunch — copy it aside first). Wait for the footer.
6. **Converged?** — the review progress file must contain `custom review complete - no more findings` or `external review found no issues`, and `tmp/ralphex-run/<plan-stem>/revmux/rounds.jsonl` must hold ≥1 line with `exit ∈ {0,1}`. `max custom iterations reached` followed by `claude review complete - no more findings` = converged-but-capped: say so. Anything else → not converged: name the last round's log and stop; never call the run reviewed.
