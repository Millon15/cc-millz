...
- 🔔 **Hook.** Fires on every SessionStart source and tells Claude to load `unslop-kit:unslop-formatting` now and again after compaction.
- 🪆 **unslop-formatting.** Loads `pstack:unslop`, runs it over the draft, then applies the layout. Chat replies get both passes; commits, PR bodies, comments, docs and Slack/Jira bodies get pass 1 only.
